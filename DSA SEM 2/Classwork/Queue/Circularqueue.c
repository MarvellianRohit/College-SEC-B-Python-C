...
void enqueue(int value) {
  ...
  if ((rear + 1) % SIZE == front) {
    ...
    // This wraps the index back to 0 if it exceeds (SIZE - 1)
...
void dequeue() {
  ...
  if (front == -1) {
    printf("Queue Underflow! Cannot delete because the queue is empty.\n"); 
    ...
    if (front == rear) {
      ...
      // Keeps the index within the bounds of [0, SIZE-1]
