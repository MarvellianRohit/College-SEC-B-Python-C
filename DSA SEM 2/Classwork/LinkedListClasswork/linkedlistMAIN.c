...
void insert_at_pos() {
  ...
  for (i = 1; i < pos - 1;
       i++) { // Navigate to the node just before the insertion position
    temp = temp->next; // Move to the next node
  }

  newnode->next =
      temp->next; // Point new node's next to the current node at this position
...
void delete_at_end() {
  struct dlinklist *temp,
      *prev; // Pointers for traversal and tracking previous node
...
// Function to delete at an intermediate position (following the algorithm
// steps)
...
void delete_at_pos() {
  ...
  if (pos == 1) {      // Case for deleting the first node
    delete_at_front(); // Call front de letion function
...
void search_node() {
  ...
  int target, pos = 1,
              found = 0; // Target value, position tracker, and status flag
  ...
  while (temp != NULL) {        // Iterate through each node in the list
    if (temp->data == target) { // Compare node data with target value
      printf("\n Value %d found at position %d.", target,
             pos); // Report location
...
void reverse_list() {
  struct dlinklist *prev = NULL, *current = start,
                   *next = NULL; // Three pointers for manipulation
...
int main() {
  ...
  while (1) { // Infinite loop for the menu system
    ...
    case 2:
      traverse();
      break; // Display list contents

    case 3:
      insert_at_front();
      break; // Insert node at start

    case 4:
      insert_at_end();
      break; // Insert node at end

    case 5:
      insert_at_pos();
      break; // Insert node at specific index
    case 6:
      delete_at_front();
      break; // Remove first node

    case 7:
      delete_at_end();
      break; // Remove last node

    case 8:
      delete_at_pos();
      break; // Remove node at specific index
    case 9:
      printf("\n Total Nodes: %d", count_nodes());
      break; // Print node count

    case 10:
      search_node();
      break; // Find value in list

    case 11:
      reverse_list();
      break; // Flip list order
